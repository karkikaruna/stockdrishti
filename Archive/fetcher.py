"""
NEPSE Live Feed Scheduler — Linux/Ubuntu

Setup (one-time):
    1. Make this script executable:
           chmod +x nepse_scheduler.py

    2. Register with cron to auto-start every day at 10:50 AM NPT:
           crontab -e
       Add this line:
           50 10 * * 1-4 /usr/bin/python3 /path/to/nepse_scheduler.py >> /path/to/nepse_data/cron.log 2>&1
       (Mon–Thu only, since NEPSE is closed Fri/Sat; Sun=0 in cron)
       Or to run every day and let the script handle day-of-week logic:
           50 10 * * * /usr/bin/python3 /path/to/nepse_scheduler.py >> /path/to/nepse_data/cron.log 2>&1

    3. Make sure the required packages are installed:
           pip3 install requests

Manual run:
    python3 nepse_scheduler.py
"""
